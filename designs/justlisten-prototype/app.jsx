...
const App = () => {
  ...
  const [mask, setMask] = useState({ x: 130, y: 260, w: 472, h: 92 });
  ...
  useEffect(() => {
    ...
    const onMove = (event) => {
      ...
      if (gesture.mode === "move") {
        setMask((current) => ({
          ...current,
          x: clamp(gesture.mask.x + dx, 8, 732 - current.w - 8),
          y: clamp(gesture.mask.y + dy, 12, 412 - current.h - 12)
        }));
      } else {
        setMask((current) => ({
          ...current,
          w: clamp(gesture.mask.w + dx, 240, 732 - current.x - 8),
          h: clamp(gesture.mask.h + dy, 42, 150)
        }));
      }
    };
  ...
  return (
    <main className="prototype-stage" data-screen-label="JustListen · Bilibili Player" style={{ "--stage-scale": stageScale }}>
      <div className="stage-canvas" ref={stageRef}>
        <img className="page-reference" src="assets/reference-page.png" alt="Bilibili 视频页面参考" />
        <div className="page-veil"></div>

        <section className="video-surface" aria-label="视频播放器">
          <img className="video-frame" src="assets/reference-player.png" alt="动画视频画面" />
          <div className="cinema-vignette"></div>
          <div className="native-subtitles">
            <div lang="en">{activeLine.en}</div>
            <div lang="zh">{activeLine.zh}</div>
          </div>
